    ...
    <main id="main" className="about-page pricing-page">
      ...
      <section className="container about-section" aria-labelledby="plans-title">
        <div className="section-head">
          ...
          <h2 id="plans-title">{locale === "zh" ? "三条产品线，十四个档位。" : "Three product lines and fourteen tiers."}</h2>
        </div>
          ...
          {plans.map((item, index) => (
            <article className={`pricing-card ${index === 0 ? "featured" : ""}`} key={`${item.family}-${item.tier}`}>
              ...
              <dl className="pricing-specs">
                <div>
                  <dt>{locale === "zh" ? "模型权益" : "Model access"}</dt>
                  <dd>{item.models[locale]}</dd>
                </div>
                <div>
                  <dt>{locale === "zh" ? "限速" : "Rate limit"}</dt>
                  <dd>{item.rateLimit[locale]}</dd>
                </div>
                <div>
                  <dt>{locale === "zh" ? "并行限制" : "Parallel limit"}</dt>
                  <dd>{item.parallelLimit[locale]}</dd>
                </div>
                <div>
                  <dt>{locale === "zh" ? "媒体生成倍率" : "Media multiplier"}</dt>
                  <dd>{item.mediaRate[locale]}</dd>
                </div>
              </dl>
            <p className="pricing-card-note">{item.note[locale]}</p>
            </article>
          ))}
        </div>
      </section>
